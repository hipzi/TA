...
$callback = function ($msg) {
    ...
    $buildqueueuser = "build_category_" . $userid . "_" . "$categoryid";
    $channel->queue_declare($buildqueueuser, false, true, false, false);
    $data = "Sedang Building Category $categoryid";
    ...
    $old_path = getcwd();
    chdir('docker');
    shell_exec("./backupBuildCategory.sh $userid $categoryid");
    chdir($old_path);

    $finishqueueuser = "finish_category_" . $userid . "_" . "$categoryid";
    $channel->queue_declare($finishqueueuser, false, true, false, false);
    $data = "Selesai Building Category $categoryid";
    $msguser = new AMQPMessage(
        $data,
        array('delivery_mode' => AMQPMessage::DELIVERY_MODE_PERSISTENT)
    );
